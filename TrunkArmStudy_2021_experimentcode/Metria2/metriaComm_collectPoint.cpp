...
//     }
    ...
    for (int jMarker = 0; jMarker < 256; jMarker++){
        if (sPacket.aMarkers[jMarker].markerSeriesNumber > 0)
        {
            ...
            *(out+ ((jCounter*8)+3)) = sPacket.aMarkers[jMarker].x;
            *(out+ ((jCounter*8)+4)) = sPacket.aMarkers[jMarker].y;
            *(out+ ((jCounter*8)+5)) = sPacket.aMarkers[jMarker].z;
            *(out+ ((jCounter*8)+6)) = sPacket.aMarkers[jMarker].qx;
            *(out+ ((jCounter*8)+7)) = sPacket.aMarkers[jMarker].qy;
            *(out+ ((jCounter*8)+8)) = sPacket.aMarkers[jMarker].qz;
            *(out+ ((jCounter*8)+9)) = sPacket.aMarkers[jMarker].qr;
            *(out+ ((jCounter*8)+10)) = sPacket.aMarkers[jMarker].markerSeriesNumber;
            jCounter++;
