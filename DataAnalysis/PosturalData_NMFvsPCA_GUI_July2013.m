...
global B Muscle_LST checkwalk time nmus ncond ntrials nbins color
...
if checkwalk==1;
    ...
    load example_postural_data B Muscle_LST
    nmus=15; ncond=1; ntrials=1; nbins=1;
...
global B Muscle_LST checkwalk time nmus ncond ntrials nbins
...
global B numsyn_nmf synergy_nmf
...
[nmus,nconds]=size(B);
    ...
    for s=1:nmus
        ...
        [synergy_nmf(s).W,synergy_nmf(s).C,synergy_nmf(s).err,synergy_nmf(s).stdev] = NNMF_stacie_May2013(B,s,1);
        [synergy_nmf(s).VAFcond, synergy_nmf(s).VAFmus, synergy_nmf(s).VAF]=funur(B,synergy_nmf(s).W,synergy_nmf(s).C); %calculate VAF of reconstruction
        synergy_nmf(s).RECON = synergy_nmf(s).W*synergy_nmf(s).C;
    end

   
%****choose number of synergies and assign variables to plot
[numsyn_nmf]=choosesyns(B,synergy_nmf);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global B Muscle_LST synergy_nmf color time checkwalk ntrials ncond nbins
...
for i=1:nsyn
    ...
    axis([0.5 nmus+.5 0 1])
    set(gca,'XTickLabel',[]);  % removes those annoying ass numbers
    if i==1, title(['Synergies']); end
    xTickLabels=Muscle_LST;
    htext=text((1:length(Muscle_LST)), -0.1*ones(1,length(Muscle_LST)), xTickLabels,'fontsize',10);
...
[COEFF,SCORE,pcvars] = pca(DATAsrc');
...
global B COEFF SCORE pcvars Muscle_LST color checkwalk time nmus ncond ntrials nbins
...
global whichtrial_NMF synergy_nmf B color Muscle_LST time checkwalk ncond ntrials nbins
...
global whichtrial_PCA COEFF SCORE B color Muscle_LST time checkwalk nmus ncond ntrials nbins
...
elseif checkwalk==1;
    ...
    for i=1:nmus
        ...
        ymax=max(B(i,:));
        rsqr = (corr(B(i,:)',ReconPCA(i,:)'))^2;
        text(100,ymax/2,['r2 =', ' ', num2str(rsqr)],'FontSize',10)
        %shows average VAF from each trial VAF b/w real and reconstructed
        ursqr = rsqr_uncentered(B(i,:),ReconPCA(i,:));
        text(100,ymax*(3/4),['VAF =', ' ', num2str(ursqr*100), ' %'],'FontSize',10)
