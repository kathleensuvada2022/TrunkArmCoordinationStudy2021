...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	/* check inputs */
	{
		if ( nrhs != 1 )
		{
			mexErrMsgIdAndTxt( "MATLAB:mexfio64:invalidNumInput",
                    "One input argument expected (temp filename).");
		}

		if ( nlhs > 0 )
		{
			mexErrMsgIdAndTxt( "MATLAB:mexfio64:maxlhs",
                    "No output arguments expected.");
		}

		if ( !mxIsChar(prhs[0]) )
		{
			mexErrMsgIdAndTxt( "MATLAB:mexfio64:invalidInput",
                    "Temp filename input argument must be a text string.");
		}

		if ( mxGetString(prhs[0], filename, sizeof(filename) / 2) )
		{
			mexErrMsgIdAndTxt( "MATLAB:mexfio64:getStringFailed",
                    "mxGetString() failed.");
		}
	}
	...
	if ( NULL == fp )
	{
		...
		if ( NULL == fp )
		{
			sprintf(str,"Failed to open/create test file '%s'",
				filename);
			mexErrMsgIdAndTxt( "MATLAB:mexfio64:openFailed", str);
			return;
		}
		else
		{
        ...
        mexPrintf("existing test file '%s' opened\n",filename);
    }
	...
	/* get file size - scroll down for getFileFstat() example */
	{
		...
		else
		{
            ...
            mexErrMsgIdAndTxt( "MATLAB:mexfio64:getFileFstatFailed",
                    "getFileFstat() failed.");
		}
	}
	...
	{
		mexfio64cleanup(fp,filename);
		mexErrMsgIdAndTxt( "MATLAB:mexfio64:setFilePosFailed",
                "setFilePos() failed.");
	}
	...
	{
		mexfio64cleanup(fp,filename);
		mexErrMsgIdAndTxt( "MATLAB:mexfio64:getFilePosFailed",
                "getFilePos() failed.");
	}
...

	{
		...
		{
			mexfio64cleanup(fp,filename);
			mexErrMsgIdAndTxt( "MATLAB:mexfio64:getFileStatFailed",
                    "getFileStat() failed.");
		}
	}
