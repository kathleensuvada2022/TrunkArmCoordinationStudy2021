...
/*==========================================================
* Dec 6, 2020 - Hendrik
* - Returned more information for each marker: the corresponding camera and related times
* - Formatted the returned data so that the each specific marker's information will always be in the same column. Previously, it was first come, first outputted. So position of data could change as markers moved between cameras.
* - Moved the data outputting and formatting outside of the main while loop to reduce time between camera pulls
*========================================================*/

//TODO: Add together sec and nsec? Are they different?
...
static void metriaComm(int sockInput, int *markerIDs, int numMarkers, int *cameraSerials, int numCameras, double *out) //12_3_2020
{
    ...
    //Create a structure array of information pulled from the cameras
    UDPPacketV101_s* cameraPackets = new UDPPacketV101_s[numCameras];
    
    //Prepare some variables that we will repeatedly use/update
    int i, j, currentCamera, pullCount = 0, pullLimit = 30;
...
//         mexPrintf("\nCamera we're looking for - index:%d, serial:%d\n",currentCamera,*(cameraSerials+currentCamera));
...
//             //Sabeen debug stuff that checks whether that pull was at all successful
//             mexPrintf("\n_socket: %d\n",_socket);
...
//                 mexPrintf("\nCamera we're looking for in char array: %s, Matches current host, bool set to %d\n",cameraSerialCharArray,cameraSerial_ReadBools[i]);
        ...
        //If this is the first camera, use this section to output starting info for the output array. I've decided against this format for now, but left it in but commented out in case it's helpful for the future.
//         if (currentCamera==0) {                                  
...
//         }
    ...
    *(out) = *(cameraSerials);             //Camera serial
    *(out+1) = cameraPackets[1].frameNumber;   //Frame number

    //Now to check through the structure array for all the relevant markers, and to format them correctly.
    for (i = 0; i<numCameras; i++) {
        //Go through the list of marker IDs we need to check and get the required info from them
        for (j = 0; j<numMarkers; j++) {
            if (cameraPackets[i].aMarkers[*(markerIDs+j)].markerSeriesNumber > 0) { //Check to see if we find a series number for this marker ID we're now checking for (lets us know there's data there)
                //Debugging printfs
    //                 mexPrintf("Marker[%04d:%03d] ", cameraPackets[i].aMarkers[*(markerIDs+i)].markerSeriesNumber, *(markerIDs+i));
    //                 mexPrintf("XYZ:[%+09.3f %+09.3f %+09.3f] ", cameraPackets[i].aMarkers[*(markerIDs+i)].x, cameraPackets[i].aMarkers[*(markerIDs+i)].y, cameraPackets[i].aMarkers[*(markerIDs+i)].z);
    //                 mexPrintf("Q:[%+08.5f %+08.5f %+08.5f %+08.5f]\n", cameraPackets[i].aMarkers[*(markerIDs+i)].qr, cameraPackets[i].aMarkers[*(markerIDs+i)].qx, cameraPackets[i].aMarkers[*(markerIDs+i)].qy, cameraPackets[i].aMarkers[*(markerIDs+i)].qz);
                if (markerID_ReadBools[j]==0) { //Do we still need data for this marker? If so, output all the relevant data   
                    //*(out+13*j) = cameraPackets[i].aMarkers[*(markerIDs+j)].markerSeriesNumber; //Marker Series     
                 //   *(out+10*j) = *(cameraSerials+i);             //Camera serial
                //    *(out+10*j+1) = cameraPackets[i].frameNumber;   //Frame number
                    *(out+8*j+2) = *(markerIDs+j);                 //Marker ID     
                    *(out+8*j+3) = cameraPackets[i].aMarkers[*(markerIDs+j)].x; //X
                    *(out+8*j+4) = cameraPackets[i].aMarkers[*(markerIDs+j)].y; //Y
                    *(out+8*j+5) = cameraPackets[i].aMarkers[*(markerIDs+j)].z; //Z
                    *(out+8*j+6) = cameraPackets[i].aMarkers[*(markerIDs+j)].qx;//qX
                    *(out+8*j+7) = cameraPackets[i].aMarkers[*(markerIDs+j)].qy;//qY
                    *(out+8*j+8) = cameraPackets[i].aMarkers[*(markerIDs+j)].qz;//qZ
                    *(out+8*j+9) = cameraPackets[i].aMarkers[*(markerIDs+j)].qr;//qR                
                    //*(out+13*j+11) = cameraPackets[i].frameTime_sec; //Frame time in OS seconds?
                    //*(out+13*j+12) = cameraPackets[i].frameTime_nsec;//Frame time in real seconds? Nanoseconds? Check this.
                    markerID_ReadBools[j]=1; //We've now found this marker and got the data for it
                }
            }
        }
    }
...
//     mexPrintf("\nNumber of cameras: %d\n",numCameras);
    ...
    int mrows = 1;
    int ncols = (numMarkers*8+2); //TODO: THIS IS VERY IMPORTANT! The number that's multiplied here is the number of parameters we're outputting from each marker. In the current case, 12
