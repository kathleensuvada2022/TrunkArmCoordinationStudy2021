function PlotKinematicData3_NRSA(partid,metriafname,act3dfname,expcond)
...
%% 
    ...
    [dist,vel,timestart]= ComputeReachStart_NRSA(afilepath,afname2)
...
%     ivelmax = timevelmax/.001;
    ...
    % Finding the EMG at all three time points for all 15 emgs
    emgstart= emg(istart,1:15)
...
%     emgmaxvel = emg(ivelmax,1:15)
    ...
    figure(3)
    PlotEMGs_NRSA(emg,dist,vel,timestart)%,title([partid '-' afname],'Interpreter','none','Position',[-2,1,0])
    pause
end
