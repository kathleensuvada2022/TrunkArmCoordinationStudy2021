...
static void metriaComm(int port2)
{
    //InitUDP BEGINS
    sockaddr_in server_addr;
    ...
    if (sPacket.aMarkers[jMarker].markerSeriesNumber > 0)
        {
            ...
            mexPrintf("\n");
//             stream << "entered the if statement!" << std::endl;
//             displayOnMATLAB(stream);
//             stream << "Marker[]: " << sPacket.aMarkers[jMarker].markerSeriesNumber << "other data piece: " << jMarker<< std::endl;
//             displayOnMATLAB(stream);
//             stream << "x: " <<  sPacket.aMarkers[jMarker].x << "y: " <<  sPacket.aMarkers[jMarker].y << "z: " <<  sPacket.aMarkers[jMarker].z << std::endl;
//             displayOnMATLAB(stream);
//                     printf("Q:[%+08.5f %+08.5f %+08.5f %+08.5f]\n", sPacket.aMarkers[jMarker].qr, sPacket.aMarkers[jMarker].qx, sPacket.aMarkers[jMarker].qy, sPacket.aMarkers[jMarker].qz);
        }
    }
    
    mexPrintf("end test :) \n");        
//     stream << "end test :)" << std::endl;
//     displayOnMATLAB(stream);
