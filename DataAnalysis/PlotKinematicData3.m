function PlotKinematicData3(partid,metriafname,act3dfname,expcond)
%
...
%added to load maxes
if exist([partid '/maxEMG.mat'])==2, load([partid '/maxEMG']);disp(maxEMG)
else
    disp('Computing Maximum Muscle EMGs. Make sure you check them')
% maxEMG=GetMaxMusAct2(flpath,basename,setfname,partid,plotflag)
    maxEMG=GetMaxMusAct2([partid '/maxes'],'MAXES','savedsetupKacey',partid,0);
end
...
%         p2=plot(nanmean([xhand(1:10,1) xshoulder(1:10,1) xtrunk(1:10,1)]),-nanmean([xhand(1:10,3) xshoulder(1:10,3) xtrunk(1:10,3)]),'o','MarkerSize',10,'MarkerFaceColor','g','MarkerEdgeColor','g');
    ...
    figure(3),PlotEMGs(emg),title([partid '-' afname],'Interpreter','none','Position',[-2,1,0])
    pause
end
figure(1)
