...
myhandles.ui.met_on = uicontrol(metPanel,'Callback',@MET_MC_Callback,'Style','checkbox','String','Enable Motion Capture','Units','Normalized','Enable','On','Interruptible','on','Position',[0.05 0.9 0.9 0.1],'FontSize',12);
myhandles.met.on = 0;
...
myhandles.ui.met_dig=uicontrol(metPanel,'Style','pushbutton','Callback',@MET_DigitizeBL_Callback,'String','<html><center>DIGITIZE<br/>BONY LANDMARKS</center></html>','Units','normalized','Position',[0.2 0.05 0.6 0.18],'FontSize',14,'BackgroundColor','y','Enable','On');

%% PPS Panel
ppsPanel = uipanel('Title','Pressure Profile Systems','FontSize',14,'FontWeight','Bold','HighlightColor','[0.5 0 0.9]','Position',[0.68,0.3,0.31,0.12]);
% Initialize Mats
...
myhandles.daq.ni.NotifyWhenDataAvailableExceeds=myhandles.daq.ni.Rate/50;
% Add listener object 
...
if myhandles.daq.rt
    ...
    daqdatabuffer=[daqdatabuffer(blocksize+1:end,:);data];
    setappdata(myhandles.daq.fig,'databuffer',daqdatabuffer)
    for i=1:nChan
    set(myhandles.daq.Line(i,1),'XData',t,'YData',daqdatabuffer(:,i));
    set(myhandles.daq.Label(i),'String',num2str([max(daqdatabuffer(:,i)) min(daqdatabuffer(:,i))],'%.3f  %.3f'));
    end
    drawnow
end

act3ddatabuffer=getappdata(act3dTACS,'databuffer');                          % KCS 8.17.20 Data Buffer? What do 
% Read current data from ACT3D
myhandles.robot.SetForceGetInfo(myhandles.exp.arm);
set(myhandles.ui.mon_eforce,'String',num2str(myhandles.robot.endEffectorForce(3))); % Vertical endpoint force
hpos=gethandpos(myhandles.robot.endEffectorPosition,myhandles.robot.endEffectorRotation,myhandles.exp); % Compute hand (3rd MCP) position
...
EXP_displayData(myhandles.daq.nChan, daqt, daqdata, myhandles.daq.sRate, [myhandles.exp.dir,'\',myhandles.exp.fname,num2str(myhandles.exp.itrial),'.mat']);
EXP_saveData([myhandles.exp.dir,'\',myhandles.exp.fname,num2str(myhandles.exp.itrial),'.mat', daqt, daqdata, ]);
...
%             get(myhandles.Line(i,2),'XData')
    ...
    % KCS added 10.30.20
    %temporary plot 
    % creating two different plots one for the sensor 1 and one for sensor
    % 2 go back to PPS to look at transpose and changing dimensions before
    % adding this 
    ...
    figure
    subplot(2,1,1)
    title('Sensor 1 Pressure Data')
    plot(myhandles.pps.data_out(1:256,:),'c')
    xlabel('Elements')
    ylabel('Samples')

    subplot(2,1,2)
    contourf(myhandles.pps.data_out(1:256,:)) % just do at one point in time 
    title('Sensor 1 Contour Data')
    xlabel('Samples')
    ylabel('Elements')
    ...
    figure
    subplot(2,1,1)
    title('Sensor 2 Pressure Data')
    plot(myhandles.pps.data_out(257:512,:),'c')
    xlabel('Elements')
    ylabel('Samples')

    subplot(2,1,2)
    contourf(myhandles.pps.data_out(257:512,:))
    title('Sensor 2 Contour Data')
    xlabel('Samples')
    ylabel('Elements')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function to initialize and start METria system Motion Capture
...
scrsz = get(groot,'ScreenSize');
digGUI = figure('Name','ACT3D_TACS - DIGITIZE BONY LANDMARKS', ...
    'NumberTitle', 'off', 'DeleteFcn',@GUI_closeDBL,'OuterPosition',[0.4*scrsz(3) 40 0.6*scrsz(3) scrsz(4)-40]);
% digGUI.Visible = 'on';
digGUI.MenuBar = 'none';
% movegui(digGUI.Fig,'center')

dig.Segments = {'Trunk (SC,IJ,PX,C7,T8)';'Scapula (AC,AA,TS,AI,PC)';'Humerus(EM,EL)';'Forearm (RS,US,OL,MCP3)'};
dig.ui.segmlist = uicontrol(digGUI,'Style','listbox','Callback',@MET_selectSEG,'String',dig.Segments,'HorizontalAlignment','Left','Units','normalized','FontSize',14,'Position',[0.2,0.8,0.6,0.1]);

dig.ui.digbutton = uicontrol(digGUI,'Style','pushbutton','Callback',@MET_CollectPB_Callback,'String','DIGIT POINT','FontWeight','Bold','FontSize',11,'Enable','Off','HorizontalAlignment','Center','Units','normalized','Position',[0.65 0.50 0.125 0.05]);
