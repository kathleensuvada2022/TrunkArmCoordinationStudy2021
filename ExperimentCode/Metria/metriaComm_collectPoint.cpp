...
//         }
    ...
    //                 mexPrintf("Q:[%+08.5f %+08.5f %+08.5f %+08.5f]\n", cameraPackets[i].aMarkers[*(markerIDs+i)].qr, cameraPackets[i].aMarkers[*(markerIDs+i)].qx, cameraPackets[i].aMarkers[*(markerIDs+i)].qy, cameraPackets[i].aMarkers[*(markerIDs+i)].qz);
                if (markerID_ReadBools[j]==0) { //Do we still need data for this marker? If so, output all the relevant data   
                    ...
                    *(out+10*j+2) = *(markerIDs+j);                 //Marker ID     
                    *(out+10*j+3) = cameraPackets[i].aMarkers[*(markerIDs+j)].x; //X
                    *(out+10*j+4) = cameraPackets[i].aMarkers[*(markerIDs+j)].y; //Y
                    *(out+10*j+5) = cameraPackets[i].aMarkers[*(markerIDs+j)].z; //Z
                    *(out+10*j+6) = cameraPackets[i].aMarkers[*(markerIDs+j)].qx;//qX
                    *(out+10*j+7) = cameraPackets[i].aMarkers[*(markerIDs+j)].qy;//qY
                    *(out+10*j+8) = cameraPackets[i].aMarkers[*(markerIDs+j)].qz;//qZ
                    *(out+10*j+9) = cameraPackets[i].aMarkers[*(markerIDs+j)].qr;//qR                
...
//     mexPrintf("\nNumber of cameras: %d\n",numCameras);
    ...
    int mrows = 1;
    int ncols = (numMarkers*10); //TODO: THIS IS VERY IMPORTANT! The number that's multiplied here is the number of parameters we're outputting from each marker. In the current case, 12
