...
% Not using Blender, remove next line in later versions
...
act3dTACS.CloseRequestFcn = @GUI_closeMain;
                                                                            ...
                                                                            % KCS 8.17.20 Can't close windows --> When edit GUi and want to rerun have to restart matlab
...
% AMA - Hard coded this
% Options: 'NI Lab/10.30.203.26' or 'MC Lab/10.30.203.36'
myhandles.room='NI Lab'; % Neuroimaging lab                                 KCS 8.14.20 need to add biodex lab option ?? 
myhandles.ipaddress='10.30.203.26';
...
myhandles.exp.ee2hLength=18; % End effector to hand distance in cm
myhandles.exp.abdAngle=90;
myhandles.exp.shfAngle=45;
myhandles.exp.elfAngle=90;
myhandles.exp.arm = 'right';                                                    % KCS 8.14.20 arm hardcoded to show on GUI? what line allows input and pressing buttons 
...
myhandles.exp.data=zeros(floor(myhandles.daq.sTime/myhandles.timer.period),myhandles.act3d.nVars);  % KCS 8.14.20 What are these two lines doing? 
...
myhandles.ui.act3d_loadtype=uicontrol(act3dPanel,'Style','popup','Tag','loadtype','String',{'N','% Abduction Max','% Arm weight'},'Units','normalized','Position',[0.55 .19 .42 0.1],'Callback',@ACT3D_loadtype_Callback);
...
uicontrol(expPanel,'Style','text','String','End effector to hand (cm)','HorizontalAlignment','left','Units','normalized','Position',[0.05,0.58,0.6,0.1],'FontSize',12);
myhandles.ui.exp_eehlength = uicontrol(expPanel,'Style','edit','String',num2str(myhandles.exp.ee2hLength),'Callback',@EXP_EELength_Callback,'Tag','EELength','HorizontalAlignment','center','Units','normalized','Position',[.65 .75 .3 .1],'UserData','right','FontSize',12);
...
%% Protocol Panel
proPanel = uipanel('Title','Experimental Protocol','FontSize',14,'FontWeight','Bold','HighlightColor','[0.5 0 0.9]','Position',[0.32,0.25,0.35,0.44]);
% File name
...
myhandles.ui.exp_go = uicontrol(proPanel,'Style','pushbutton','Callback',@EXP_Go_Callback,'HorizontalAlignment','center','String','GO! (Press "g")','Units','normalized','Position',[0.2 0.23 0.6 0.17],'FontWeight','Bold','FontSize',12,'Enable','Off','BackgroundColor','g');
% File save directory
...
%% Monitor Panel
monPanel = uipanel('Title','Signal Monitor','FontSize',14,'FontWeight','Bold','HighlightColor','[0.5 0 0.9]','Position',[0.32,0.01,0.35,0.23]);
...
myhandles.ui.mon_eforce=uicontrol(monPanel,'Style','text','String','N/A','HorizontalAlignment','left','Units','normalized','Position',[0.56,0.74,.4,.18],'FontSize',14,'ForegroundColor','b');
% Endpoint position
uicontrol(monPanel,'Style','text','String','Endpoint position (m)','HorizontalAlignment','left','Units','normalized','Position',[0.05,0.51,.5,.18],'FontSize',12);
myhandles.ui.mon_epos=uicontrol(monPanel,'Style','text','String','N/A','HorizontalAlignment','left','Units','normalized','Position',[0.56,0.51,.4,.18],'FontSize',14,'ForegroundColor','b');
% Shoulder position
uicontrol(monPanel,'Style','text','String','Shoulder position (m)','HorizontalAlignment','left','Units','normalized','Position',[0.05,0.28,.5,.18],'FontSize',12);
myhandles.ui.mon_spos=uicontrol(monPanel,'Style','text','String','N/A','HorizontalAlignment','left','Units','normalized','Position',[0.56,0.28,.4,.18],'FontSize',14,'ForegroundColor','b');
% Arm weight
uicontrol(monPanel,'Style','text','String','Arm weight (N)','HorizontalAlignment','left','Units','normalized','Position',[0.05,0.05,.5,.18],'FontSize',12);
myhandles.ui.mon_awgt=uicontrol(monPanel,'Style','text','String','N/A','HorizontalAlignment','left','Units','normalized','Position',[0.56,0.05,.4,.18],'FontSize',14,'ForegroundColor','b');
...
% guidata(fighandle,myhandles)
                                                                            ...
                                                                            % KCS 8.18.20 In visual Disp need midline? cone? 
...
set(hAxes,'xlim',[-0.4 0.4],'ylim',[-0.05 1.15*larm]); % Want both axes to be 0-centered, y-axis equal to 120% arm length, x-axis 80 cm.
hLine=zeros(5,1);
...
hLabel = uicontrol(hFig,'Style','text','BackgroundColor','k','ForegroundColor','w','HorizontalAlignment','center','Units','normalized','FontSize',32.5,'Position',[0.4 0.9 0.2 0.09]);
...
%     delete(myhandles.judp)
    if isfield(myhandles,'nidaq'), delete(myhandles.nidaq); end
...
% Prevent users from closing DAQ window before closing program
    warndlg('Close DAQ display window by disabling DAQ or closing program');
end

function GUI_closeEXPDisp(source,event)
% Prevent users from closing DAQ window before closing program
    warndlg('Close user feedback display window by closing program');
end
...
function ACT3D_Timer_Callback(hObject,event)
    ...
    robot.SetForceGetInfo(myhandles.exp.arm);
    ...
    if ~strcmp(robot.currentState,myhandles.act3d.state)
        warndlg(['Warning: ACT3D current state is ' robot.currentState '. Power off device and re-initialize'])
    ...
    myhandles.ui.mon_eforce=num2str(robot.endEffectorForce(3)); % Vertical endpoint force
    hpos=gethandpos(robot.endEffectorPosition,robot.endEffectorRotation,myhandles.exp); % Compute hand (3rd MCP) and shoulder position
    myhandles.ui.mon_epos=mat2str(hpos,1);

    % Update display feedback
                                                                             % KCS 8.18.20 - makes move with feedback? How is this working? Using
                                                                             % blender? 
    ...
    databuffer=[data;databuffer(1:end-1,:)];
...
switch hObject.String
    case 'INIT'
        ...
        myhandles.haptic.isHorizontalCreated = haptic.Create(myhandles.haptic.horizontalName,myhandles.haptic.isHorizontalCreated,...
        ...
        myhandles.timer.ExecutionMode = 'queue'; % Changed to 'queue' from 'fixedrate' mode so that period is on average the set period.
    ...
    switch myhandles.act3d.table
        case 'on'
            ...
            if robot.endEffectorPosition(3) < haptic.horizontalPosition(3)
                wh=warndlg('Please raise the ACT3D above the haptic table','Trunk Arm Interaction','modal');
                uiwait(wh)
            end
            myhandles.haptic.Enable(myhandles.haptic.isHorizontalCreated,myhandles.haptic.isHorizontalEnabled,myhandles.haptic.horizontalName);
        case 'off'
            % turn off horizontal effect
            myhandles.haptic.isHorizontalEnabled = haptic.Disable(myhandles.haptic.isHorizontalCreated,myhandles.haptic.isHorizontalEnabled,myhandles.haptic.horizontalName );
    end
% guidata(fighandle,myhandles)
    ...
    switch myhandles.act3d.loadtype
        case 'max'
            if force>100 || force<0 
                warndlg('Warning: % Maximum Abduction Force should be between 0 and 100');
                return
            end
        ...
        case 'wgh'
            if force>100 || force<0 
                warndlg('Warning: % Arm Weight should be between 0 and 100');
                return
            end
            ...
            myhandles.exp.endforce=-force*abs(myhandles.exp.arm_weight)/100 - myhandles.exp.arm_weight;
        case 'abs'
    ...
    switch source.Value
        ...
        case 2
            if ~isfield(myhandles.exp,'max_sabd')
                warndlg('Warning: Measure or Enter Maximum Shoulder Abduction Force first')
%                 myhandles.ui.act3d_loadbg.SelectedObject=myhandles.ui.act3d_absload;
        ...
        case 3
            if ~isfield(myhandles.exp,'arm_weight')
                warndlg('Warning: Measure Arm Weight first')
%                 myhandles.ui.act3d_loadbg.SelectedObject=myhandles.ui.act3d_absload;
    ...
    switch myhandles.act3d.loadstate
        case 'on'
            ...
            if myhandles.haptic.isHorizontalEnabled
                warndlg('Warning: Haptic table will be moved down 6 cm')
                myhandles.haptic.Disable(myhandles.haptic.isHorizontalCreated,myhandles.haptic.isHorizontalEnabled,myhandles.haptic.horizontalName);
                myhandles.haptic.horizontalPosition(3) = -0.16;
            ...
            robot.SetExternalForce([0 0 myhandles.exp.endforce]);    
        case 'off'
            robot.SetExternalForce([0 0 0]);    
            if myhandles.haptic.isHorizontalEnabled
                myhandles.haptic.Disable(myhandles.haptic.isHorizontalCreated,myhandles.haptic.isHorizontalEnabled,myhandles.haptic.horizontalName);
                myhandles.haptic.horizontalPosition(3) = -0.1;
                % set position of horizontal haptic effect in robot
                myhandles.haptic.SetPosition(myhandles.haptic.horizontalPosition,myhandles.haptic.horizontalName);
                wh=warndlg('Warning: Haptic table will be moved to default position, move ACT-3D above table','Trunk Arm Interaction','modal');
                uiwait(wh)
                myhandles.haptic.Enable(myhandles.haptic.isHorizontalCreated,myhandles.haptic.isHorizontalEnabled,myhandles.haptic.horizontalName);
            end
    end        
...
function DAQ_Init_Callback(source,event)
    ...
    if isfield(myhandles.daq,'s'), delete(myhandles.daq.s); myhandles=rmfield(myhandles.daq,'s'); end
    % Create DAQ session
    myhandles.daq.s = daq.createSession('s');
    ...
    myhandles.daq.s.Rate = myhandles.daq.sRate;
    myhandles.daq.s.DurationInSeconds = myhandles.daq.sTime;
    % Add listener object
    lh = addlistener(myhandles.daq.s, 'DataAvailable', @DAQ_localTimerAction);
...
function [hAxis,hLine,hLabel]=DAQ_createFig(nChan,yaxlabel)
    if ~isfield(myhandles.ui,'daqFig')
        myhandles.ui.daqFig = figure('Name','EMG DAQ','NumberTitle','off','OuterPosition',[0.25*scrsz(3) 40 0.5*scrsz(3) scrsz(4)-40]);
        myhandles.ui.daqFig.MenuBar = 'none';
        myhandles.ui.daqFig.CloseRequestFcn =  @GUI_closeDAQDisp;
    else
        figure(myhandles.ui.daqFig)
    end
...
% Set end effector to hand (3rd MCP) distance in cm
function EXP_EELength_Callback(hObject,event)
    myhandles.exp.ee2hLength=str2double(hObject.String);
end
...
myhandles.pro.itrial=1; myhandles.ui.exp_itrial.String=num2str(myhandles.pro.itrial);
...
if exist([myhandles.exp.dir '\' myhandles.exp.fname num2str(myhandles.pro.itrial) '.mat'],'file')==2
    ...
    if strcmp(response,'No')
        ...
        def={myhandles.exp.fname num2str(myhandles.pro.itrial)};
        dlgTitle='Input File Name';
        lineNo=1;
        answer=inputdlg(prompt,dlgTitle,lineNo,def);
        myhandles.exp.fname=answer{1};set(source,'String',myhandles.exp.fname);
        myhandles.pro.itrial=str2num(answer{2}); set(myhandles.ui.exp_itrial,'String',num2str(myhandles.pro.itrial))
    end
end
end
% Display and set the trial number
function EXP_iTrial_Callback(hObject,event)
myhandles.pro.itrial = str2num(hObject.String);
if exist([myhandles.exp.dir '\' myhandles.exp.fname num2str(myhandles.pro.itrial) '.mat'],'file')==2
    response=questdlg('The data file already exists. Do you want to overwrite it?','ACT3D - Trunk Arm Coordination Study','Yes','No','No');
    if strcmp(response,'No')
        prompt={'Enter the file name:','Enter the trial number:'};
...
% Prompt user to place endpoint in participant's midline
uiwait(msgbox('Place tip of middle finger in front of sternum and click OK','ACT3D - Trunk Arm Coordination Study','modal'));
if strcmp(myhandles.act3d.state,'FIXED')
    ACT3D_Init_Callback(myhandles.ui.act3d_state,[])
end

% switch the robot to a fixed state to keep the participant's arm still if
% not already in fixed state
...
if strcmp(myhandles.act3d.state,'NORMAL')
    ...
    myhandles.ui.mon_spos=num2str(robot.endEffectorForce(3)); % Vertical endpoint force
    myhandles.ui.mon_awgt=num2str(robot.endEffectorForce(3)); % Vertical endpoint force
    set(myhandles.exp.hLine(3),'Position',[-0.05 -0.05 0.1 0.1]); % home target
end
